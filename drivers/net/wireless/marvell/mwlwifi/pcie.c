 ...
 * Some portions Copyright (C) 2017, Laird
 *
...
/* This file implements the PCIe interface chip-specific functions. */
...
static int mwl_rx_init(struct ieee80211_hw *hw)
{
	struct mwl_priv *priv = hw->priv;
	int rc;
...
static void mwl_rx_deinit(struct ieee80211_hw *hw)
{
	struct mwl_priv *priv = hw->priv;
...
static void mwl_pcie_card_reset(struct mwl_priv *priv)
{
	...
	if (mwl_pcie_check_card_status(priv))
		writel(ISR_RESET,
		       card->iobase1 + MACREG_REG_A2H_INTERRUPT_EVENTS);
}
		...
		unsigned short cmd)
{
	...
	do {
		int_code = le16_to_cpu(*((__le16 *)&priv->pcmd_buf[INTF_CMDHEADER_LEN(INTF_HEADER_LEN)+0]));
...
static irqreturn_t mwl_pcie_isr(int irq, void *dev_id)
{
	...
	int_status_mask = card->iobase1 + MACREG_REG_A2H_INTERRUPT_STATUS_MASK;

	int_status = readl(card->iobase1 + MACREG_REG_A2H_INTERRUPT_CAUSE);
...
static void mwl_pcie_cleanup(struct mwl_priv *priv)
{
	...
	if (pdev) {
		/* TODO: reading of devm_ioremap_resource doc says iounmap isn't necessary */
// 		iounmap(&pdev->resource[0]);
// 		iounmap(&pdev->resource[card->next_bar_num]);
		pci_disable_device(pdev);
		pci_set_drvdata(pdev, NULL);
	}
...
static int mwl_pcie_register_dev(struct mwl_priv *priv)
{
	...
	tasklet_init(&priv->tx_task, (void *)mwl_tx_skbs, (unsigned long)priv->hw);
	...
	tasklet_init(&priv->rx_task, (void *)mwl_pcie_rx_recv, (unsigned long)priv->hw);
	tasklet_disable(&priv->rx_task);

	tasklet_init(&priv->qe_task,
		     (void *)mwl_pcie_tx_flush_amsdu, (unsigned long)priv->hw);
...
static struct mwl_if_ops pcie_ops = {
	...
	.flush_amsdu			= mwl_pcie_tx_flush_amsdu,
	.dbg_info			= mwl_pcie_dbg_info,
	.dbg_reg_access			= mwl_pcie_debugfs_reg_access,
};

static int mwl_pcie_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (id->driver_data >= MWLUNKNOWN) /* uses MWLMAX? */
		return -ENODEV;
...
static void mwl_pcie_remove(struct pci_dev *pdev)
{
...
static struct pci_driver mwl_pcie_driver = {
	.name     = MWL_DRV_NAME,  /* FIXME: will this conflict? */
	.id_table = mwl_pci_id_tbl,
	.probe    = mwl_pcie_probe,
	.remove   = mwl_pcie_remove,
};
