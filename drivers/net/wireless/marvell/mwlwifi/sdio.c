 ...
 */
static int mwl_read_reg(struct mwl_priv *priv, u32 reg, u8 *data)
{
 ...
 */
static int mwl_write_reg(struct mwl_priv *priv, u32 reg, u8 data)
{
	struct mwl_sdio_card *card = priv->intf;
	int rc = 0;
...
void *mwl_alloc_dma_align_buf(int rx_len, gfp_t flags)
{
	struct sk_buff *skb;
	int buf_len, pad;
...
void mwl_sdio_send_command(struct mwl_priv *priv)
{
	struct mwl_sdio_card *card = priv->intf;
 ...
 * function is invoked.
 */
static void mwl_decode_rx_packet(struct mwl_priv *priv,
	...
	if (sdio_ireg & UP_LD_CMD_PORT_HOST_INT_STATUS) {
		...
		*/
		if ((card->cmd_cond == false) &&
		    ((cmd_hdr->command & ~HOSTCMD_RESP_BIT) == card->cmd_id) &&
		    (priv->pcmd_buf[1] ==  cpu_to_le16(MWL_TYPE_CMD))) {
			card->cmd_id = 0;
			mwl_sdio_complete_cmd(priv);
		} else if (priv->pcmd_buf[1] ==  cpu_to_le16(MWL_TYPE_EVENT))
			mwl_sdio_event(priv);
	}
...
void mwl_handle_rx_packet(struct mwl_priv *priv, struct sk_buff *skb)
{
	...
	*/
	...
	qos_ctrl = pdesc->qos_ctrl;
	/*
	    Remove the DMA header (dma->fwlen)
	*/
	mwl_rx_remove_dma_header(prx_skb, qos_ctrl);
 ...
 */
static void mwl_write_data_complete(struct mwl_priv *priv,
				    struct sk_buff *skb)
{
	struct mwl_sdio_card *card = priv->intf;
			    ...
			    struct sk_buff *skb)
{
	...
	/* FIXME: skb_cow to avoid kernel oops. need to identify and fix specific packet */
	skb_cow(skb, sizeof(struct mwl_tx_desc));
	...
	tx_desc->pkt_ptr = ((u8 *)skb->data - ptr);
	tx_desc->status = 0;
	return;
}
...
static int mwl_sdio_init_irq(struct mwl_priv *priv)
{
	...
	wiphy_info(priv->hw->wiphy, "%s, register IRQ\n", __func__);

	sdio_claim_host(func);
	/* Request the SDIO IRQ */
	ret = sdio_claim_irq(func, mwl_sdio_interrupt);
	if (ret) {
		wiphy_err(priv->hw->wiphy, "claim irq failed: ret=%d\n", ret);
		goto out;
	}
...
err_add_card:
	destroy_workqueue(card->tx_workq);
	sdio_ops.ptx_work = NULL;
	sdio_ops.ptx_workq = NULL;
...
module_driver(mwl_sdio_driver, sdio_register_driver, sdio_unregister_driver);

MODULE_DESCRIPTION("Marvell 802.11ac Wireless SDIO Network Driver");
MODULE_AUTHOR("Marvell Semiconductor, Inc.");
MODULE_LICENSE("GPL v2");

MODULE_FIRMWARE(SD8864_DEFAULT_FW_NAME);
