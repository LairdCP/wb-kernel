...
static void inf_btsdio_err_mem_record(struct inf_bt_if *bt_if, int err, bool set, u32 addr, u32 size)
{
	struct bts_cmd_entity *cmd = NULL;
...
static void inf_btsdio_int_handler(struct sdio_func *func)
{
	struct brcmf_bus *bus_if = NULL;
	struct brcmf_sdio_dev *sdiodev = NULL;
	struct inf_bt_if *bt_if = NULL;
...
void inf_btsdio_init(struct brcmf_bus *bus_if)
{
	...
	if (!pdata->oob_irq_supported) {
		...
		sdio_claim_irq(sdiodev->func3, inf_btsdio_int_handler);
	}
	sdio_release_host(sdiodev->func1);
